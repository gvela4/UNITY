...
public class PlayerAttackEffects : MonoBehaviour {
    ...
    void GroundImpact()
    {
        Instantiate(groundImpact_Prefab, groundImpact_Spawn.transform.position, Quaternion.identity); // rotation 000
    }
    void Kick()
    {
    ...
    void FireShield()
    {
        ...
        fireObj.transform.SetParent(transform); // so we can exactly position this fireObj under the root player
    }
    void Heal()
    {
    ...
    void ThunderAttack()
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            if (i == 0) // setting where the position for the current number
            {
