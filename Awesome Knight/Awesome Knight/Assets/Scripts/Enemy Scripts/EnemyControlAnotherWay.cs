...
public class EnemyControlAnotherWay : MonoBehaviour
{
    ...
    void MoveAndAttack()
    {
        ...
        if (distance > walk_Distance)
        {
            ...
            {
                navAgent.isStopped = false; // we are not stopping
        ...
        else
        {
            ...
            {
                navAgent.isStopped = true; // we dont need the enemy to walk, enemy will attack
                anim.SetBool("Run", false);
                Vector3 targetPosition = new Vector3(playerTarget.position.x, transform.position.y, playerTarget.position.z);
                // to rotate the enemy from one position to the other
                ...
                {
                    anim.SetInteger("Atk", 0);
                    currentAttackTime += Time.deltaTime; // deltaTime time in seconds it took to complete last frame
                }
            }
        }
    }
