...
public class FPSShootingControls : NetworkBehaviour
{
    ...
    void Shoot()
    {
        if (Input.GetMouseButtonDown(0) && Time.time > nextTimeToFire) // allows fire
        {
            ...
            if (Physics.Raycast(mainCam.transform.position, mainCam.transform.forward, out hit))
            {
                ...
                //print("Position of game object is:" + hit.transform.position);
                //print("Point where hit happended is:" + hit.point);

                // hit point is where we actually hit with the raycast
    ...
    [Command] // attribute for network behavior
